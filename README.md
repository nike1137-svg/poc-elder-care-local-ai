...
|---|---|---|---|---|
| 문자오류율 CER | 악화 3%p 이내 | **0.0131** | 0.0075 ~ 0.0185 | ✅ |
| 처리 속도 | 실시간 1배속 이내 | **RTF 0.38** | RTF 1.82 ~ 2.13 | ✅ |
| 건당 비용 | 0원 | **0원** | 약 0.16원 | ✅ |
| 외부 전송 | 0건 | **0건** | 19건 | ✅ |
| 특이사항 재현율 | 70% 이상 | **1.000** | — | ✅ |

**기존 방식의 CER이 범위인 이유가 이 프로젝트에서 가장 흥미로운 발견이다.**
같은 음성을 하루 간격으로 두 번 돌렸더니 0.0075와 0.0185가 나왔다 — 한 번은 이기고 한 번은 졌다.
**로컬은 두 번 돌린 출력이 글자 하나까지 같았다.** 정확도를 따지기 전에
"몇 점인지 말할 수 있는가"에서 갈린다. → [상세](docs/evaluation.md)
...
| 기준 | 목표 | 개선안(로컬) | 기존 1차 | 기존 2차 | |
|---|---|---:|---:|---:|:-:|
| **S1** 문자오류율 | 악화 3%p 이내 | **0.0131** | 0.0075 | 0.0185 | ✅ |
| **S2** 처리 시간 | 실시간 1배속 이내 | **RTF 0.38** | 1.82 | 2.13 | ✅ |
| **S3** 비용 | 0원 | **0원** | 약 3원 | 약 3원 | ✅ |
| **S4** 외부 전송 | 0건 | **0건** | 19건 | 19건 | ✅ |
| **S5** 특이사항 재현율 | 70% 이상 | **1.000** | — | — | ✅ |

**다섯 항목 모두 달성했다.**

### 기존 방식을 두 번 쟀더니 값이 달라졌다

같은 음성 20건, 같은 프롬프트, `temperature=0`. 하루 간격으로 두 번 돌렸는데
**CER이 0.0075와 0.0185로 2.5배 벌어졌다.** 1차에서는 클라우드가 이겼고, 2차에서는 졌다.
...
원인을 가리려고 **버전이 고정된 모델을 몇 분 간격으로 두 번** 돌려봤다.
그 사이에 모델이 바뀔 리는 없으니, 결과가 같으면 "모델이 교체된 것"이 된다.

**결과는 4건 전부 동일했다.** → 별칭(`-latest`)이 가리키는 모델이 바뀌었을 가능성이 높다.
다만 무료 한도 때문에 비교 표본이 4건뿐이라 단정하지는 않는다.

반면 **로컬 모델은 두 번 돌린 출력이 글자 하나까지 같았다**(해시 일치 확인).

> 정확도 우열을 따지기 전에 **"몇 점인지 말할 수 있는가"**에서 갈린다.
> 돌봄 서비스에서 *"어제는 알아듣던 말을 오늘은 못 알아듣는"* 일이 생겨도
> 클라우드 쪽은 원인을 짚을 수가 없다.

그리고 **두 방식이 같은 단어에서 같이 틀렸다.** "시큰거려서"를 클라우드는 "시끈거려서",
로컬은 "식흥거려서"로 적었다. 도메인 어휘 문제는 클라우드로 옮긴다고 풀리지 않는다 —
...
| 발견 | 조치 | 효과 |
|---|---|---|
| **기준선을 다시 쟀더니 값이 달랐다** (CER 0.0075→0.0185) | 버전 고정 모델로 대조 실험 → `modelVersion` 기록 추가 | **결론이 뒤집힐 뻔한 것을 잡음** |
...
| README대로 따라가면 **저장소를 받는 단계가 없었다** | 다른 기계에서 문서만 보고 따라해봄 | 클론 단계 추가 |

> 전부 **실측·테스트·직접 따라해보기**에서 나왔다. 문서를 읽어서는 하나도 안 나왔다.
>
> 특히 맨 위 항목이 그렇다. 기준선을 한 번만 재고 "클라우드가 0.56%p 더 정확하다"로
> 끝냈다면, **그 문장은 다음 날 틀린 문장이 됐을 것이다.**
> 좋게 나온 숫자를 한 번 더 의심한 게 이 프로젝트에서 제일 값진 습관이었다.
...
- **표본이 작다.** 합성 20건 + 공개셋 40건. 1%p 안팎의 CER 차이는 표본 오차와 구분되지 않는다
- **정확도 우열을 단정하지 못했다.** 클라우드 CER이 실행마다 달라져(0.0075 ↔ 0.0185)
  두 번 중 한 번씩 나눠 가졌다. "어느 쪽이 정확하다"고 말할 근거가 부족하다
- **기준선 측정에 실서비스와 같은 모델을 못 썼다.** `gemini-2.5-flash`가 신규 사용자에게
  더 이상 제공되지 않아 현행 flash 별칭으로 대체했다 — 그리고 **그 별칭을 쓴 것 자체가
  재현성을 해쳤다.** 벤치마크에는 버전 고정 이름을 썼어야 했다
- **변동의 원인을 확정하지 못했다.** "모델 교체" 쪽에 무게가 실리지만,
  무료 한도 때문에 비교 표본이 4건뿐이었다
...
- 표본이 20건이다 → 1%p 안팎의 CER 차이는 표본 오차와 구분되지 않는다
- 기준선을 실서비스와 동일한 모델로 재지 못했다
- **기준선 자체가 흔들린다** → 클라우드 CER이 실행마다 달라져,
  "얼마나 나아졌는가"를 한 숫자로 못 박을 수가 없다

마지막 항목은 이 PoC의 한계이자 **동시에 결론이기도 하다.**
비교 대상이 내 손을 벗어나 있으면, 개선 폭을 재는 것부터가 흔들린다.
로컬로 내리면 그 문제가 사라진다 — 어제 잰 값이 오늘도 같다.
